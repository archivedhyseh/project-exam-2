...
export default function NavMenu({ setIsOpen }: NavMenuProps) {
  return (
    <div
      className="fixed left-0 top-0 flex h-full w-full justify-end bg-black/30 p-4"
            ...
            {routes.map((route, index) => {
              return (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    isActive
                      ? 'inline-flex rounded-full py-2 font-bold text-text'
                      : 'inline-flex rounded-full py-2 text-text hover:text-text-muted'
                  }
                >
                  {route.title}
                </NavLink>
